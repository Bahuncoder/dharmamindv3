...
const AuthComponent: React.FC<AuthComponentProps> = ({ 
  ...
  // Handle forgot password within modal context
  const handleForgotPassword = () => {
    // For now, we can either:
    // 1. Add forgot password mode to this modal
    // 2. Show a simple alert for future implementation
    // 3. Keep the current navigation for complex flows
    if (onClose) onClose();
  ...
  return (
    <div className="modal-content rounded-2xl shadow-2xl w-full max-w-md p-8 relative">
      {/* Close Button */}
      {onClose && (
        <button
          onClick={onClose}
      ...
      {/* General Error */}
      {errors.general && (
        <div className="mb-4 p-3 bg-error-bg border border-error rounded-lg">
      ...
      <div className="mb-6">
        <button
          ...
          className="btn-outline w-full inline-flex justify-center py-3 px-4 border rounded-lg shadow-sm text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300"
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={isSubmitting || isLoading}
          className="btn-primary w-full py-2 px-4 rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <div className="mt-8 pt-6 border-t border-medium">
          ...
          <div className="space-y-2">
            <button
              ...
              className="btn-success w-full py-2 px-4 rounded-lg transition-colors disabled:opacity-50 text-sm"
            >
              🆓 Demo Mode (Free)
            </button>
          </div>
