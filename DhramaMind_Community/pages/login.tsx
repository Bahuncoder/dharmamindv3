<<<<<<< HEAD
/**
 * Community Login - Redirects to Central Auth (Brand Webpage)
 */
...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    const handleAuth = async () => {
      ...
      if (communityAuth.isAuthenticated()) {
        ...
        if (currentUser) {
          setUser(currentUser);
          // Redirect to community or intended page
          const returnUrl = router.query.returnUrl as string || '/community';
  ...
  const handleCentralAuthRedirect = () => {
    ...
    const returnUrl = router.query.returnUrl as string || '/community';
    const fullReturnUrl = `${currentUrl}/login?returnUrl=${encodeURIComponent(returnUrl)}`;
    
    // Redirect to Chat App (Central Auth Hub) with return URL
    const centralAuthUrl = `https://dharmamind.ai/auth?mode=login&returnUrl=${encodeURIComponent(fullReturnUrl)}&platform=community&source=${encodeURIComponent(currentUrl)}`;
  ...
  const handleRegisterRedirect = () => {
    ...
    // Redirect to Chat App register
    const centralRegisterUrl = `https://dharmamind.ai/auth?mode=signup&returnUrl=${encodeURIComponent(fullReturnUrl)}&platform=community&source=${encodeURIComponent(currentUrl)}`;
    window.location.href = centralRegisterUrl;
  };

  // Show loading while redirecting to central auth
  return (
    <>
      <Head>
            ...
            <div className="relative z-10">
              ...
              <div className="mb-8">
                <div className="relative inline-block">
                  <div className="w-20 h-20 bg-primary-gradient rounded-2xl flex items-center justify-center mx-auto mb-4">
                ...
                <p className="text-lg font-medium text-gray-600">Community Platform</p>
              </div>

              {/* Loading Message */}
              <div className="mb-8">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-emerald-600 mx-auto mb-4"></div>
                <h2 className="text-2xl font-bold text-gray-800 mb-4">Connecting to DharmaMind</h2>
              ...
              <div className="mt-8 pt-6 border-t border-gray-200/50">
                ...
                <div className="flex justify-center gap-2 text-xs">
                  <span className="px-2 py-1 bg-emerald-100 text-emerald-700 rounded-full">Community</span>
                  <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded-full">Main Site</span>
          ...
          __html: `
            ...
            setTimeout(function() {
              if (window.location.pathname === '/login') {
                window.location.href = 'https://dharmamind.ai/auth?mode=login&returnUrl=${encodeURIComponent(window.location.href)}&platform=community';
              }
            }, 3000);
          `
        }}
